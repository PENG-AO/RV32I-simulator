...
const s32 cache_get_valid_block(const CACHE* cache, const u32 addr) {
...
const s32 cache_get_empty_block(const CACHE* cache, const u32 addr) {
...
const s32 cache_get_replacable_block(const CACHE* cache, const u32 addr) {
...
const u32 cache_read_word(CACHE* const cache, const u32 addr, u32* const val) {
...
const u32 cache_write_word(CACHE* const cache, const u32 addr, const u32 val) {
    ++cache->reference_counter;
    ++cache->write_counter;
    register const s32 block_idx = cache_get_valid_block(cache, addr);
    if (block_idx < 0) {
...
void cache_load_block(CACHE* const cache, const u32 addr, const MEM* mem) {
...
u32 cache_sneak(CACHE* cache, u32 addr, u32* val) {
